...
export function Timeline({ rows, tz, windowLabel, onOpen, onFill }: {
  rows:        ShapeRow[]
  tz:          string
    ...
    <section className="min-w-0">
      ...
      <div className="rounded-xl border border-line bg-surface overflow-hidden divide-y divide-line-soft">
        {rows.map(row => row.kind === 'event'
          ? <EventRow key={row.key} row={row} tz={tz} />
...
function EventRow({ row, tz }: { row: Extract<ShapeRow, { kind: 'event' }>; tz: string }) {
